...
public class ProjectService : IProjectService
{
    ...
    public async Task<ProjectDTO> CreateProjectAsync(ProjectCreateDTO dto)
    {
        ...
        if (existingProject != null)
            throw new InvalidOperationException($"Ya existe un proyecto con el nombre '{dto.Name}'");
    ...
    public async Task<bool> DeleteProjectAsync(Guid id)
    {
        ...
        var hasReservedOrSoldLots = project.Blocks
            .SelectMany(b => b.Lots)
            .Any(l => l.Status == LotStatus.Reserved || l.Status == LotStatus.Sold);

        if (hasReservedOrSoldLots)
            throw new InvalidOperationException("No se puede eliminar un proyecto que tiene lotes reservados o vendidos");
