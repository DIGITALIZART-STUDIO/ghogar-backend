...
public class ProjectService : IProjectService
{
    ...
    public async Task<PaginatedResponseV2<ProjectDTO>> GetAllProjectsPaginatedAsync(
        int page,
        int pageSize,
        string? search = null,
        string? orderBy = null,
        ...
        )
        {
            ...
            if (page == 1)
            {
                ...
                var preselectedProject = await _context
                    .Projects.Include(p => p.Blocks)
                    .ThenInclude(b => b.Lots)
                    .FirstOrDefaultAsync(p => p.Id == preselectedGuid);
        ...
        if (!string.IsNullOrWhiteSpace(orderBy))
        {
            ...
            if (preselectedGuid.HasValue && page == 1)
            {
                query = orderBy.ToLower() switch
                {
                    ...
                    _ => query
                        .OrderBy(p => p.Id == preselectedGuid ? 0 : 1)
                        .ThenByDescending(p => p.CreatedAt), // Ordenamiento por defecto
                };
            }
            else
            {
